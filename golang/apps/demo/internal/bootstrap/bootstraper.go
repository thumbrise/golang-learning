...
func (b *Bootstrapper) Bootstrap(ctx context.Context, modules []contracts.Module) error {
	for _, mm := range modules {
		m := mm
		b.logger.Log(ctx, "Module", m.Name(), "got", nil)
	}
	...
	for _, mm := range modules {
		...
		b.logger.Log(ctx, "Module", m.Name(), "before start", err)
	}
	...
	for _, mm := range modules {
		...
		err := m.OnStart(ctx)
		b.logger.Log(ctx, "Module", m.Name(), "on start", err)
	}
...
func (b *Bootstrapper) Shutdown(ctx context.Context, modules []contracts.Module) error {
	for _, mm := range modules {
		...
		err := m.Shutdown(ctx)
		b.logger.Log(ctx, "Module", m.Name(), "shutdown", err)
	}
